...
func (a adapter) PreCheck(req *ReqForCheck) (status int64, description string, rawInfo map[string]string, err error) {
	...
	hash:=GetSha256(a.userid+"."+string(req.Datatime.String()),[]byte(a.Password))
	req.Hash=hash
	req.UserID=a.userid
...
func (a adapter) Payment(req *ReqForPayment) (status int64, description string, paymentID string, err error){
...
func (a adapter) PostCheck(req *ReqForPayment) (status int64, description string,err error){
