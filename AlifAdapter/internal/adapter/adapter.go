...
func (a adapter) PreCheck(account string, serviceID string) (status int64, description string, rawInfo map[string]string, err error) {
    ...
    data,err:=json.Marshal(Req{Service:serviceID,UserID: a.userid,Hash: "",Account: account,Amount: "",Currency: "TJS",TxnID: "",Phone: "",
...
func (a adapter) Payment(account string,serviceID string,amount string,trnID string, notifyRoute string) (status int64, description string, paymentID string, err error){
	...
	temp,err:=json.Marshal(Req{Service:serviceID,UserID: a.userid,Hash: "",Account: account,Amount: "",Currency: "TJS",TxnID: "",Phone: "",
	Fee: "",Providerid: "0",Last_Name: "",First_Name: "",Middle_Name: "",Sender_Birthday: "",Address: "",Resident_Country: "",Postal_Code: "",Recipient_Name: ""})
	if err!=nil{
		logrus.WithError(err).Warn("on marshaling")
		return 0,"","0",err 
...
func alifCheck(account,serviceID,alifUrl,userid string,httpClien *http.Client) (status int64, description string, rawInfo map[string]string, err error){

    data,err:=json.Marshal(Req{Service:serviceID,UserID: userid,Hash: "",Account: account,Amount: "",Currency: "TJS",TxnID: "",Phone: "",
	Fee: "",Providerid: "0",Last_Name: "",First_Name: "",Middle_Name: "",Sender_Birthday: "",Address: "",Resident_Country: "",Postal_Code: "",Recipient_Name: ""})
	if err!=nil{
		logrus.WithError(err).Warn("on marshaling")
		return 0,"0",rawInfo,nil
	...
	// encodedBody := data.Encode()
	req,err:=http.NewRequest(http.MethodGet,alifUrl,bytes.NewBuffer(data))
	if err!=nil{
		logrus.WithError(err).Warn("on create request")
		return 0,"0",rawInfo,nil
	}
...
func (a adapter) PostCheck(trnID string,account string,serviceID string) (status int64, description string,err error){
	data:=Req{Service:serviceID,UserID: a.userid,Hash: "",Account: account,Amount: "",Currency: "TJS",TxnID: trnID,Phone: "",
	Fee: "",Providerid: "0",Last_Name: "",First_Name: "",Middle_Name: "",Sender_Birthday: "",Address: "",Resident_Country: "",Postal_Code: "",Recipient_Name: ""}
	if err!=nil{
