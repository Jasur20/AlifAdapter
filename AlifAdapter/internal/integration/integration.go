...
type Adapter interface {
	...
	Payment(req *adapter.ReqForPayment) (status int64, description string, paymentID string, err error)
	PostCheck(req *adapter.ReqForPayment) (status int64, description string, err error)
}
...
func (i integration) ReceiverInfo(operID string, req *GetReceiverInfoRequestBody) *GetReceiverInfoResponseBody {
	...
	datatime := carbon.Parse(carbon.Now().String()).ToRfc7231String()
	reqBody := adapter.ReqForCheck{Account: req.Account, Service: req.ProviderServiceID, Datatime: datatime, Currency: "TJS"}
...
func (i integration) Payment(operID string, req *PaymentRequestBody) *PaymentResponseBody {
	...
	reqBody := adapter.ReqForPayment{Service: req.ProviderServiceID, TxnID: string(req.ID), Amount: amount, Currency: req.ReceiverBilAccCurrency, Account: req.Account}
...
func (i integration) PostCheck(operID string, req *PostCheckRequestBody) *PostCheckResponseBody {
	...
	reqBody := adapter.ReqForPayment{Account: req.Account, Service: req.ServiceID, TxnID: string(req.ID)}
